...
- 개인키 암호방식 (대칭 암호 기법, 단일키 암호화 기법)
    - *동일한 키로 데이터를 암호화하고 복호화* 함
    - 비밀키는 DB 사용 권한이 있는 사용자만 나눠 가짐
    - DES, SEED, AES, ARIA, LFSR, RC4
- 공개키 암호방식 (비대칭 암호 기법)
...
2. OWASP (the Open Web Application Security Project)
    - 웹 정보 노출이나 악성 코드, 스크립트, 보안이 취약한 부분 연구

3. TCP 래퍼 TCP wrapper
    - 외부 컴퓨터의 접속 인가 여부를 점검하여 접속 허용 및 거부
    - 다른 컴퓨터 사용자의 ID 및 로그를 조회하여 악용하는 것 막는 방화벽 역할
...
5. DPI (Deep Packet Inspection)
    - OSI 7계층의 프로토콜과 패킷 내부 컨텐츠를 파악하여 침입 시도